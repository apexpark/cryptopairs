...
| Updated by | claude |
| Repo HEAD pin (committed) | `7041b41f2430fdd8cf02fe24c29e84784e6a502a` |
| Pin branch | `main` |
| Sprint base branch | `main` |
| Pin notes | State refreshed after PR #246 made the merge-authority tiers operative with hardened Tier 1–2 delegation and the expanded protected-path list. AUTO-2 posture unchanged since PR #242: constrained to the paper-autopilot sequence — static paper trial, shadow dynamic champion/challenger allowlist, governed dynamic allowlist, dynamic paper trial, then live-design gate only. Future coding slices must pass the Slice Loop Check before implementation. |
...
| GOV-SCAFFOLD-3 - CLAUDE.md Autonomy Doctrine | **In flight** | claude | Branch `claude/claude-md-autonomy-doctrine`. Creates `CLAUDE.md` (Claude session entry point + Autonomy Doctrine: operator-invoked/evidence-gated phase, per-component graduation on the native AUTO ladder with AUTO-3 never grantable by the doctrine, OP-1 Q3 safety invariants, never-do list), wires it into `.agentic/project.yaml`, and records the PR #246 merge authorization. `CLAUDE.md` and `.agentic/**` are protected — Tier 3: Codex exact-SHA review + Operator authorization. Remaining after this: GOV-SCAFFOLD-4 (CODEOWNERS expansion to match the decisions-register protected list). |
