...
| Slice E — observability and runbooks | **Docs groundwork proposed** | Agent 3 | Branch `codex/reoptimise-runner-observability-docs` adds operator-facing runbook groundwork only: bounded metric/label guidance, missing-telemetry fail-closed behavior, stuck lease / budget / artifact / cancellation / unsafe-promotion alert guidance, enable/disable/cancel/rollback flows, and explicit operator-only host verification boundary. No Rust, metric implementation, scheduler enablement, or host-state claim. |
