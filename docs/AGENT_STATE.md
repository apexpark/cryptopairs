...
| Updated by | claude |
| Repo HEAD pin (committed) | `c1eaf79ca7ce5f39514b4a8133a875bdbcc54794` |
| Pin branch | `main` |
...
| AUTO-2B.2 - B2-a selector-view contracts | **In flight** | claude | Branch `claude/b2a-selector-view-contracts`. observe_record v2 (oneOf: unchanged entry rows + new selector-view row type, SELECTOR_VIEW_OBSERVED, no outcome fields), snapshot v2 optional selector_view/universe/per-stream-churn blocks, new selector-view example, example-validation test. Contracts only — no tool code (B2-b/c). Tier 3 flow required. |
| GOV-ROLESWAP - Swap Claude/Codex roles (OP-44) | **In flight** | claude | Branch `claude/swap-coder-reviewer-roles`. Codex → Lead Coder + Operator Interface; Claude → Independent Reviewer. Operative for the first slice after this merges; B2-b PR #252 completes under prior roles. Adds `CODEX.md`; updates ai_workflow/git-github/CLAUDE role sections, codex_prompt_pack note, CODEOWNERS, project.yaml. Tier 3: authored by Claude (still Coder until merge), reviewed by Codex (still Reviewer until merge). |
