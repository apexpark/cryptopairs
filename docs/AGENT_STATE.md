...
| Pin notes | State refreshed after PR #245 merged the dual-agent governance scaffold v0 (`.agentic/**`). Merge-authority tiers become operative when the workflow-manual amendment slice (GOV-SCAFFOLD-2) merges. AUTO-2 posture unchanged since PR #242: it remains constrained to the paper-autopilot sequence. AUTO-2 remains constrained to the paper-autopilot sequence: static paper trial, shadow dynamic champion/challenger allowlist, governed dynamic allowlist, dynamic paper trial, then live-design gate only. Future coding slices must pass the Slice Loop Check before implementation. |
