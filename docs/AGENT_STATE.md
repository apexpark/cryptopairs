...
| X3 | low | After Slice C lands and legacy-row behavior is observed, align reporting/diagnostic surfaces that still expose only legacy `selected_variant` (backtest, live-z, paper-trades, opportunity-history) so operators can distinguish evaluated-best vs stored-champion presentation without breaking existing contracts. | open — deferred until after Slice C host/runtime behavior is implemented and observed; expected shape is additive diagnostics or docs, not removal/redefinition of legacy `selected_variant`. |
...
3. **Remote/local agent: Slice D planning** — after Slice C observation data is captured, design recanonicalization of legacy selected rows without making unsafe legacy provenance trade-eligible.
4. **Remote/local agent: X3 reporting alignment** — after Slice C lands and is observed, add compatible reporting/diagnostic alignment for backtest, live-z, paper-trades, and opportunity-history surfaces if still needed.
5. **Operator action (long-term cleanup)**: PR the full agent-docs chain from `codex/fix-clippy-run-24549051096` to `main` when ready, then flip Sprint base branch in §Pin to `main`.
