...
| Slice D — async API and script migration | **Async endpoint subset proposed** | Agent 6 | Proposed delta: PR branch `codex/slice-d-async-api-endpoints` adds read/enqueue-only async run endpoints on top of Slice C durable state. Enqueue fails closed while the disabled-by-default async worker is off, and an enabled worker claims existing queued API runs before scheduling its own run. The existing synchronous `/v1/strategy/pairs/reoptimize` route remains unchanged; script migration, UI changes, production scheduler defaults, automatic promotion/revert, repair-provenance graduation, artifact download routes, and mutating cancellation remain deferred. Cancellation is explicitly blocked until the operator approves an auth/audit boundary. |
