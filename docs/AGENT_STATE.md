...
| Updated by | codex |
| Repo HEAD pin (committed) | `88f12eab5ec29c19b91a27497c1658c6cf109002` |
| Pin branch | `main` |
| Sprint base branch | `main` |
| Pin notes | Pin is `origin/main` after PR #255 merged AUTO-2B.2 B2-c. The completed Operator-run B2-d evidence window then exposed one narrow producer/consumer value-set mismatch: B2-a-valid selector rows carry the strategy service's non-actionable `direction_hint="NONE"`, while merged B2-c rejected that value. AG-20260724-011 repairs only that consumer boundary. |
| Origin | `https://github.com/apexpark/cryptopairs.git` |
| Working-tree state | **B2-cR is implemented and Codex inner-review CLEAN under AG-20260724-011; a fresh Claude exact-SHA review is the next gate.** The test-only checkpoint fails exactly the two new `NONE` ingestion/determinism expectations against merged B2-c; the repair passes 30 focused tests plus 21 subtests and the full canonical `tools/scripts` suite passes 198 tests plus 70 subtests. Inner review found and repaired an attempted global direction-set expansion before commit: `NONE` is now accepted only on selector-view inputs, while realized paper-event directions remain restricted to `LONG_SPREAD` / `SHORT_SPREAD`. OBS-1, OBS-3, and CI-1 remain open. No producer, contract, capture, host, service, eligibility, scoring-policy, deploy, secret, trading, AUTO-2C, or unattended-loop work is included. |
...
| AUTO-2B.2 - B2-c shadow scorer selector-view input | **Done — PR #255 merged** | codex (author) / claude (reviewer) | Claude reviewed final PR head `6d8d57b74998cb918fdf8bb8ff22be58f8565852` CLEAN. The Operator authorized the Tier 3 squash merge after the exact-head, required-check, and unresolved-thread gates; PR #255 landed on `main` as `88f12eab5ec29c19b91a27497c1658c6cf109002`. B2-c consumes strict complete selector ticks into advisory v2 selector/universe/churn output and cannot control paper entries. The completed B2-d production-shaped evidence later exposed the separately scoped `NONE` consumer mismatch addressed by B2-cR. |
| AUTO-2B.2 - B2-d Operator selector-view evidence pass | **Capture complete; end-to-end validation blocked on B2-cR** | operator | Run `20260720T000558Z` completed naturally at Git head `88f12eab5ec29c19b91a27497c1658c6cf109002`. The accepted final transcript records four unchanged shards, 849 complete manifests, 13,584 selector rows, no malformed lines, duplicate ticks, incomplete manifests, or trailing partials. The follow-up diagnostic found 11,418 schema-valid `NONE` rows, all in `WATCHLIST` or `EXCLUDED`, while every `TRADE_NOW` row used `LONG_SPREAD` or `SHORT_SPREAD`. Capture integrity passed, but merged B2-c rejected `NONE`; B2-cR is required before a separately authorized replay. No recapture is required or authorized. |
| AUTO-2B.2 - B2-cR direction-sentinel consumer repair | **Ready for fresh independent exact-SHA review** | codex (author) / claude (reviewer) | Branch `codex/b2cr-direction-sentinel-repair` repairs only the B2-d-evidenced consumer defect at merged base `88f12eab5ec29c19b91a27497c1658c6cf109002`: selector-view `direction_hint="NONE"` remains a distinct non-actionable key; null retains its prior meaning; unknown selector directions still fail closed; realized paper-event directions remain restricted to `LONG_SPREAD` / `SHORT_SPREAD`. Mutation evidence: the test-only checkpoint `1e3a8585ad6d4bc93834f4f5c2c024fdeb586693` fails the two new expectations against merged B2-c. Repaired evidence: 30 focused tests + 21 subtests and 198 full tests + 70 subtests pass. Codex inner review found and corrected the paper-direction scope leak before commit. Fresh Claude review, green CI, and explicit Operator authorization remain required before merge. No producer, contract shape, capture, host action, eligibility, scoring-policy, service, deploy, secret, trading, OBS-1/OBS-3, AUTO-2C, or unattended-loop work is included. |
...
Operator-approved queue per **OP-45**, updated by the production-shaped B2-d
evidence. Do not skip a gate:

Completed prerequisites: B2-b merged as `04826d1`; B2-c PR #255 was
Claude-reviewed CLEAN at `6d8d57b74998cb918fdf8bb8ff22be58f8565852`
and Operator-authorized, landing as
`88f12eab5ec29c19b91a27497c1658c6cf109002`. The Operator-run B2-d capture
`20260720T000558Z` then completed naturally with 849 manifests and 13,584
structurally complete selector rows. Its validation found the narrow
B2-a-producer/B2-c-consumer `NONE` mismatch; recapture is neither required nor
authorized.

1. **Claude: independently review B2-cR at its exact PR head** — verify the
   selector-only `NONE` acceptance, unchanged paper direction set, null
   distinction, unknown-value fail-closed behavior, evidence segregation, and
   tests. A fresh CLEAN verdict binds only that exact SHA.
2. **Operator: decide the B2-cR merge only after CLEAN, green required checks,
   and zero unresolved review threads** — Tier 3; no merge is pre-authorized.
3. **Separately gated Operator-only validation of the preserved B2-d evidence**
   — after B2-cR lands and the host checkout is deliberately aligned, replay
   the existing four shards through offline B2-c artifact scoring, writing
   only a separately authorized new snapshot/report. This is not authorization
   for host access, file creation, recapture, eligibility changes, service
   changes, paper/live trading, or any unattended loop.
4. **Codex: prepare the AUTO-2C design proposal only after that repaired replay
   is accepted** — use the captured evidence to design the governed dynamic
   allowlist governor; do not begin implementation.
5. **Stop after the AUTO-2C proposal** and rebuild the implementation queue
   from the accepted evidence (OP-45(e)); do not auto-continue into AUTO-2C
   implementation, AUTO-2D, or AUTO-3.
