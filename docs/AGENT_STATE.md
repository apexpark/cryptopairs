...
| Updated by | claude |
| Repo HEAD pin (committed) | `2516fc5dcf4d030b32684ad87257301e59d7dc89` |
| Pin branch | `main` |
| Sprint base branch | `main` |
| Pin notes | State refreshed after PR #245 merged the dual-agent governance scaffold v0 (`.agentic/**`). Merge-authority tiers become operative when the workflow-manual amendment slice (GOV-SCAFFOLD-2) merges. AUTO-2 posture unchanged since PR #242: constrained to the paper-autopilot sequence — static paper trial, shadow dynamic champion/challenger allowlist, governed dynamic allowlist, dynamic paper trial, then live-design gate only. Future coding slices must pass the Slice Loop Check before implementation. |
...
| GOV-SCAFFOLD-2 - Make merge-authority tiers operative | **In flight** | claude | Branch `claude/workflow-tiers-operative`. Amends `docs/ops/ai_workflow.md` (Merge Authority Tiers section, tier-scoped review protocol, role mapping), adds a merge-tier declaration to the PR template, adds the Tier 3 exact-SHA reviewer prompt to `docs/ops/codex_prompt_pack.md`, and records the PR #245 merge authorization plus the OP-7 standing Tier 1-2 delegation (hardened per adversarial inner review: green-checks-verified merges only, per-merge record comment, named forbidden file categories) in the decisions register. Also expands the Tier 3 protected-path list per Operator ratification OP-8 (all `services/**`, `scripts/**`, `docs/playbooks/**`, `.githooks/**`, `docker-compose*.yml`, dependency/toolchain manifests, `.env.example`) and records the OP-8 Tier 2 review-standard confirmation. Docs-only; touches `.github/**` and `.agentic/**`, so Tier 3: Codex exact-SHA review + Operator authorization required. |
