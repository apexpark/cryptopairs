...
| R3-impl | low-medium | Implement the merged R3 proposal: replace the `SKIP_RUST_CHECKS=1` early return in `.githooks/pre-push`, extend `scripts/test-pre-push.sh`, update remote-agent bootstrap docs, and record the operator-tooling changelog entry. | **implementation in review on `codex/r3-preflight-override-impl`** — operator §9 decisions applied: hard reject `SKIP_RUST_CHECKS=1`; reject exact boolean-ish `RUST_PREFLIGHT_OVERRIDE` values `1`, `true`, `TRUE`, `yes`, `YES`; print supplied reasons exactly with docs warning not to include secrets; keep sentinel-file override future-only. No repo pin bump in this PR. |
