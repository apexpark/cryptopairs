...
| R3-impl | low-medium | Implement the merged R3 proposal: replace the `SKIP_RUST_CHECKS=1` early return in `.githooks/pre-push`, extend `scripts/test-pre-push.sh`, update remote-agent bootstrap docs, and record the operator-tooling changelog entry. | **implementation in review in PR #172 on `codex/r3-preflight-override-impl`** — operator §9 decisions applied: hard reject `SKIP_RUST_CHECKS=1`; reject exact boolean-ish `RUST_PREFLIGHT_OVERRIDE` values `1`, `true`, `TRUE`, `yes`, `YES`; print supplied reasons exactly with docs warning not to include secrets; keep sentinel-file override future-only. No repo pin bump in this PR. |
| X1 | low | Audit script in `docs/27` §"Live Cue Mismatch Audit" still reads `cue.selected_variant` and `cue.selected_signal_config.source`. Update to use `cue.selection_state` once Slice A is on the host. | **proposed resolved by this PR** — audit command now reads `cue.selection_state.best_variant`, `cue.selection_state.stored_champion_variant`, `cue.selection_state.source`, and `cue.selection_state.validation_state`, with `missing_selection_state_count` surfacing hosts not yet serving the Slice A cue contract. |
...
2. **Operator action: R3 §9 answers** — decide hard reject vs deprecation window for `SKIP_RUST_CHECKS=1`, validation/sanitization for `RUST_PREFLIGHT_OVERRIDE`, and whether sentinel-file override stays future-only.
3. **Remote agent: R3-impl** — after operator answers, implement the focused hook/test/docs/changelog patch. No Rust code expected.
...
6. **Remote/local agent: Slice C implementation** — only after host lineage import and operator decisions; implement neutral champion selection behind the approved rollout path, preserve Slice A/B semantics, and add B6 pg-backed tests.
7. **Remote/local agent: Slice D planning** — after Slice C observation data is captured, design recanonicalization of legacy selected rows without making unsafe legacy provenance trade-eligible.
