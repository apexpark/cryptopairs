...
| AUTO-2B.2 - B2-b selector-view capture | **In flight** | claude | Branch `claude/b2b-selector-view-capture`. `autopilot_observe.py` gains disabled-by-default `AUTOPILOT_OBSERVE_CAPTURE_SELECTOR_VIEW` (all three cue buckets → v2 selector-view rows, no outcome/eligibility path), a `MAX_RUNTIME_SECONDS` bound, a per-tick row count, and a runbook section requiring a read-only disk estimate before capture. Disabled-default behaviour is byte-identical (tested). Next: B2-c (shadow scorer selector-view input), B2-d (operator capture window). Tier 3 flow. |
