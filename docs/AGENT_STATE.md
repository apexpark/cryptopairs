...
| R3-impl | low-medium | Implement the merged R3 proposal: replace the `SKIP_RUST_CHECKS=1` early return in `.githooks/pre-push`, extend `scripts/test-pre-push.sh`, update remote-agent bootstrap docs, and record the operator-tooling changelog entry. | **blocked on operator §9 decisions** — answer hard reject vs deprecation window, boolean-ish/short-reason validation, reason printing/sanitization, and whether sentinel-file override should remain future-only. Once answered, remote Codex can implement a focused non-Rust PR. |
