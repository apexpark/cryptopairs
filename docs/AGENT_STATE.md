...
| Updated by | claude |
| Repo HEAD pin (committed) | `b409849bd9b673958f5347d15be013c488e14974` |
| Pin branch | `main` |
| Sprint base branch | `main` |
| Pin notes | State refreshed after PR #247 added the CLAUDE.md Autonomy Doctrine on the full AUTO-2 §3 ladder. AUTO-2 posture unchanged since PR #242: constrained to the paper-autopilot sequence — static paper trial, shadow dynamic champion/challenger allowlist, governed dynamic allowlist, dynamic paper trial, then live-design gate only. Future coding slices must pass the Slice Loop Check before implementation. |
...
| GOV-SCAFFOLD-4 - CODEOWNERS canonicalization | **In flight** | claude | Branch `claude/codeowners-expansion`. Expands `.github/CODEOWNERS` to the full OP-8 protected-path list plus retained legacy protections and declares it the single source of truth for Tier 3; records the PR #247 merge authorization. Consequence recorded: Tier 1–2 delegated PRs touch neither `CHANGELOG.md` nor `docs/AGENT_STATE.md`. Final scaffold slice — closes the apex-forge audit gaps. Tier 3 flow required. |
